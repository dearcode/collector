...
int str_find(char *src, char *key, char *val, char *sep1, char *sep2)
{
	...
	ps = strstr(src, key);
	while (ps) {
		M_cirinl(str_separate(&ps, pk, sep1)) if (strcmp(pk, key) == 0) {
...
int comm_gets(char *src, char *sb, char *se, char *dest, int dlen)
{
	...
	if (len > 0) {
		if (len > dlen) {
			worker_set_error("Dest buffer is smaller than new string, dest len:%d, new:%d.", dlen, len);
			return MRT_ERR;
		}
...
int move_gets(char **src, char *sb, char *se, char *dest, int dest_size)
{
	...
	if (len < 1 || len > dest_size) {
		worker_set_error("new string length error, len:%d, dest size:%d", len, dest_size);
		return MRT_ERR;
	}
	strncpy(dest, psb + blen, len);
	*src = pse + elen;
