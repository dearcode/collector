...
int event_insert(conn_t * conn)
{
	...
	if (epoll_ctl(ec->epfd, EPOLL_CTL_ADD, conn->fd, &epev) == MRT_ERR) {
		log_error("epoll_ctl Add error:%m, addr:(%s), fd:%d, event:%d", conn->addr_str, conn->fd, conn->wait);
		return MRT_ERR;
	}
	...
	log_debug("%x from:%s fd:%d set event:%d", conn->id, conn->addr_str, conn->fd, conn->wait);
...
//不管函数执行成功失败都会清空ec
int event_center_init(int max_conn, int timeout, char *host, int port, callback_t on_accept,	//在接收完连接时调用
