...
void generate_path(char *buf, size_t size, char *prefix, char *suffix)
{
	...
	ntm = localtime(&now);
	snprintf(buf, size, "%s/%d/%.2d/%.6x%.4x%s", prefix, ntm->tm_year + 1900, m, (uint32_t) now, inc, suffix);
...
void generate_filename(char *buf, size_t size)
{
	...
	snprintf(buf, size, "%x%x", num1, num2);
}
...
int file_open_temp(char *path, file_handle_t * file)
{
	...
	for (i = 0; i < 100; i++) {
		gettimeofday(&tv, NULL);
		snprintf(file->from, sizeof(file->from), "%s/%.4x.%.4x.%.2x", path, getpid(), (uint32_t) tv.tv_usec, i);
