...
int check_update(cw_site_t *site, html_page_t *page)
{
    ...
    {
        log_error("no found hot part start:%s, end:%s, url:%s", site->hot_part_begin, site->hot_part_end, site->url);
        return MRT_ERR;
    }
...
int fetch_article_url(cw_site_t *site, cw_class_t *class, html_page_t *page)
{
    ...
    while(string_move_fetch(&list, ltag->href_begin, ltag->href_end, &part) == MRT_SUC)
    {
...
int fix_content_image(html_page_t *page, string_t *content)
{
    ...
    while((string_move_fetch(content, "<img", ">", &simg) == MRT_SUC) || (string_move_fetch(content, "<IMG", ">", &simg) == MRT_SUC))
    {
...
int fetch_article_content(cw_site_t *site)
{
    ...
    while((content = M_list_first(site->content_list)))
    {
        do
        {
            ...
            {
                log_error("string_fetch caption error:%s url:%s, src:%s",
            ...
            {
                log_error("No found desc part in url:%s, part start:%s, end:%s.",
            ...
            {
                log_error("string_fetch content error:%s url:%s, part start:%s, end:%s.",
...
//            log_debug("will fix content image, url:%s", content->url_real.str);
        ...
        M_list_remove_head(site->content_list);
...
int fix_class_info(cw_site_t *site, html_page_t *page)
{
    ...
    M_list_foreach(class, site->class_list)
    {
...
int fetch_class(cw_site_t *site, html_page_t *page)
{
    ...
    if(string_move_fetch(&page->data, ct->part_begin, ct->part_end, &part) == MRT_ERR)
    {
    ...
    while(html_move_fetch_href(&part, &url, &caption) == MRT_SUC)
    {
        ...
        M_list_insert_head(site->class_list, class);
...
int check_web(cw_site_t *site)
{
    ...
    M_list_init(site->content_list);
    M_list_init(site->class_list);
