...
int distribute_begin(T_ftp_session *ftp, T_ftp_host *host)
{
    ...
    {
        log_error("ftp session init errror, addr:%s:%d, user:%s, pass:%s",
...
int send_html_list(T_jsp_list *jlst, T_ftp_session *ftp)
{
    ...
    M_list_foreach(jsp, jlst)
    {
        ...
        {
            log_error("ftp process file error, local:%s, remote:%s/%s",
                      local_file, ftp->target_folder, jsp->file);
            return MRT_ERR;
        }

        log_info("ftp process file success, local:%s, remote:%s/%s",
                 local_file, target_folder, jsp->file);
    }
...
int distribute_proc_class(T_ftp_session *ftp, T_jsp_list *jlst)
{
    ...
    {
        if(java_create_html(JSP_class_url, server.class_ary[i], jlst) == MRT_ERR)
        {
