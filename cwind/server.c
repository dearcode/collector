...
int load_site_info(mydb_t *mdb, cw_site_t *site)
{
    ...
    {
        log_error("Load game lisite->tags error, site id:%d, name:%s.",
    ...
    {
        log_error("Load game info tags error, site id:%d, name:%s.",
                  site->site_id, site->name);
        return MRT_ERR;
    }
...
int server_start(void *dat)
{
    ...
    M_list_foreach(site, server.site_list)
    {
...
int main(int argc, char *argv[])
{
    ...
    string_printf(&str, "%d %d %s %s", 1, 2, "3", "4");
    ...
    */
    ...
    _exit(0);
    callback_t proc;
    callback_set(proc, server_start, NULL);
    M_ciril(worker_create(proc), "create event single loop error.");

    while(1)
        sleep(1000);

    /*
    if(server_start() == MRT_ERR)
    {
        printf("%s server start error.\n", __func__);
        return MRT_ERR;
    }
