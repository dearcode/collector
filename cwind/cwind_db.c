...
int save_site(cw_site_t *site)
{
    ...
    sprintf(sql, "update site_info set site_md5='%s' where site_id = %d",
            site->md5, site->site_id);
...
int load_remote_mysql_info(mydb_t *mysql_info, cw_site_t *site)
{
    ...
    {
        log_error("mydb_query error, sql:%s, error:%s.",
                  sqlcmd, mysql_error(&mysql_info->srv));
        return MRT_ERR;
    }
...
int load_content_tag(mydb_t *mdb, cw_site_t *site)
{
    ...
    sprintf(sqlcmd, "select part_begin, part_end, href_begin, href_end, "
            ...
            "content_end from content_tag where content_id=%d",
            site->content_id);
...
int load_list_tag(mydb_t *mdb, cw_site_t *site)
{
    ...
    sprintf(sqlcmd, "select list_part_begin, list_part_end, href_part_begin, href_part_end, "
            "href_begin, href_end, caption_begin, caption_end, page_part_begin, page_part_end, "
            "next_caption from list_tag where list_id=%d",
            site->list_tag_id);
...
int load_filter(mydb_t *mdb, cw_site_t *site)
{
    ...
    M_list_init(&site->filter->remove_list);
    M_list_init(&site->filter->replace_list);
    ...
    while((row = mysql_fetch_row(res)))
    {
        if(atoi(row[0]) == 0)
        {
            ...
            pmk->new_len = 0;
            M_list_insert_head(&site->filter->remove_list, pmk);
            r1++;
        }
        else
        {
            ...
            M_list_insert_head(&site->filter->replace_list, pmk);
            r2++;
        }
    }
...
int load_site_list(mydb_t *mdb, cw_site_list_t **list)
{
    ...
    while((row = mysql_fetch_row(res)))
    {
        ...
        log_info("site_id:%s, site_name:%s, site_url:%s, site_md5:%s, site_level:%s",
                 row[0], row[1], row[2], row[3], row[4]);

        M_list_insert_head(site_list, site_info);
        site_list->all_sum++;
    }
...
int content_info_save(cw_site_t *site, cw_content_t *ct)
{
    ...
    M_ciril(string_printf(&cmd, "call cwindPushArticle('%s', '%s', '%s')", ct->second_class.str, ct->caption.str, ct->content.str),
            "string print sql error.");
